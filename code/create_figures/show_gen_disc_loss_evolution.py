...
def show_losses_evolution(data_df, client_id, epoch=1000, step_size=10, folder='./', show_full_line = True): # data_acc, data_fid,  image_path, epoch):
    ...
    ax1.set_ylabel('Generator loss', color=color, fontsize=16)
    #ax1.set_ylim(0,200)
    ax1.plot(x, gen_loss[:epoch], color=color)
    ax1.tick_params(axis='y', labelcolor=color, labelsize=14)
    ax2 = ax1.twinx()  # instantiate a second axes that shares the same x-axis
    color = 'tab:blue'
    ax2.set_ylabel('Discriminator loss', color=color, fontsize=16)  # we already handled the x-label with ax1
    #ax2.set_ylim(0, 100)
    ax2.plot(x, disc_loss[:epoch], color=color)
    ax2.tick_params(axis='y', labelcolor=color, labelsize=14)
    fig.tight_layout()  # otherwise the right y-label is slightly clipped
    if show_full_line: plt.xlim(0, int(len(data_gen_loss)/ step_size))
    plt.savefig(folder + '-gen_vs_disc_looss-' + str(client_id) + '.png')
    plt.show()
...
def create_df_from_log_file(filename):
    ...
    log_file = '/home/jamal/Documents/Research/modeling-air-quality/lipizzaner-gan/src/output/lipizzaner_gan/distributed/gaussian/2020-09-19_15-30-54/3635/lipizzaner_2020-09-19_15-30.log'
...
def get_all_csvs(folder):
    import glob
    return [filepath for filepath in glob.iglob(folder + '/*.csv')]

def create_images_folder(folder, csv_file):
    folder_name = csv_file.split('lipizzaner_')[-1]
    folder_name = folder + 'lipi_' + str(folder_name[:-4])
...
if use_log_file:
    ...
    print(len(losses_df))
    show_losses_evolution(losses_df, 1, epoch=len(losses_df), step_size=1) #, data_acc, data_fid,  image_path, epoch):

if use_csv_file:
    csv_files = get_all_csvs(data_evolution_folder)
    for csv_file in csv_files:
        lossesdf_list = get_list_of_df_from_csv(csv_file)
        folder = create_images_folder(images_folder, csv_file)

        for client_id, losses_df in enumerate(lossesdf_list):
            try:
                show_losses_evolution(losses_df, client_id, 500, 1, folder)
            except:
                print('Error: When showing {} data.'.format(csv_file))
...
show_evolution('/home/jamal/Documents/Research/sourcecode/evaluate-lipizzaneer-output/data/evolution/mnist-fid-evolution-lipizzaner_2020-10-14_08-29.csv')